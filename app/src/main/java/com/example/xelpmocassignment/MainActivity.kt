package com.example.xelpmocassignment
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        addButton.setOnClickListener {
            if (input.text == null) {
                Toast.makeText(this, "Enter a non-zero value", Toast.LENGTH_SHORT).show()
                
            }
            val num: Float = input.text.toString().toFloat()
            if (num == 0F) {
                Toast.makeText(this, "Enter a non-zero value", Toast.LENGTH_SHORT).show()
                input.text.clear()
            } else if(num!= 0F) {
